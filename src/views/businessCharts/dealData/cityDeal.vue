<template>
  ...
  <div class="tab-container">
    ...
    <el-table :data="tableData" border style="width: 100%" :header-cell-style="{background:'rgba(240,240,240,.5)'}" :cell-style="{align:'center'}" >
      ...
      // 导出数据
          import('@/libs/map/Export2Excel.js').then(excel => {
            ...
            if (this.tableData == null || this.tableData.length == 0) {
              ...
              // 要导出的数据
              const excelHeader = ['日期', '收支', '类型', '对方账户', '金额（GAC）', '订单号', '备注/会员名'];
              const relationField = ['bill_time', 'payFlag', 'billType', 'peerNo', 'quan', 'orderNo', 'remark'];
